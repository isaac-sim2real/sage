...
class SimulationComparator:
    ...
    def _load_simulation_data(self, folder):
        ...
        data["dof_state"] = self._process_dof_data(
            data["state_motor"], data["joints"], ["positions", "velocities", "torques"]
        )
    ...
    def align_data(self, joint_name, data_type="positions"):
        ...
        max_time = min(baseline_df["time_since_zero"].max(), gaponet_df["time_since_zero"].max())
    ...
    def calculate_metrics(self, joint_name, data_type="positions"):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            if valid_indices.sum() == 0:
                ...
                abs_percentage_errors = np.abs(
                    (filtered_gaponet["value"] - filtered_baseline["value"]) / filtered_baseline["value"]
                )
        ...
        correlation = np.corrcoef(aligned_baseline["value"], aligned_gaponet["value"])[0, 1]
        metrics["correlation"] = correlation

        # 4. Cosine similarity
        similarity = 1 - distance.cosine(aligned_baseline["value"], aligned_gaponet["value"])
        metrics["cosine_similarity"] = similarity

        # 5. Cross-correlation for lag analysis
        corr = signal.correlate(aligned_baseline["value"], aligned_gaponet["value"], method="fft")
        corr /= np.max(corr)
        ...
        metrics["max_abs_error"] = np.max(np.abs(aligned_baseline["value"] - aligned_gaponet["value"]))

        # 8. Mean absolute error
        metrics["mae"] = np.mean(np.abs(aligned_baseline["value"] - aligned_gaponet["value"]))
        ...
        if data_type == "torques":
            ...
            if metrics["baseline_rms"] > 0:
                metrics["rms_change_percent"] = (
                    (metrics["gaponet_rms"] - metrics["baseline_rms"]) / metrics["baseline_rms"] * 100
                )
            ...
            if data_type == "torques":
                ...
                summary[data_type]["torque_rms"] = {
                    ...
                    "change_percent": float(
                        (np.mean(gaponet_rms) - np.mean(baseline_rms)) / np.mean(baseline_rms) * 100
                    ),
                }
    ...
    def _create_visualizations(self, all_metrics, output_path):
        ...
        for i, data_type in enumerate(data_types):
            rmse_values = [all_metrics[data_type][joint]["rmse"] for joint in joints]
    ...
    def _plot_time_series_comparison(self, joint_name, output_path):
        ...
        for i, (data_type, label) in enumerate(zip(data_types, labels)):
            ...
            axes[i].plot(
                aligned_gaponet["timestamp"], aligned_gaponet["value"], "r--", label="GapONet", linewidth=1.5, alpha=0.7
            )
...
def main():
    parser = argparse.ArgumentParser(description="Advanced comparison of GapONet vs Baseline simulation results")
    parser.add_argument("--baseline", type=str, required=True, help="Path to baseline simulation output folder")
    parser.add_argument("--gaponet", type=str, required=True, help="Path to GapONet simulation output folder")
    parser.add_argument(
        "--output",
        type=str,
    ...
    parser.add_argument(
        "--sample-freq", type=int, default=200, help="Sampling frequency in Hz for data alignment (default: 200)"
    )
    ...
    comparator = SimulationComparator(
        baseline_folder=args.baseline, gaponet_folder=args.gaponet, sample_dt=1.0 / args.sample_freq
    )

    # Run analysis
    comparator.analyze_all_joints(args.output)
