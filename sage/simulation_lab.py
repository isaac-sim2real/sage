...
import numpy as np
import torch
from isaaclab.actuators import GapONetActuatorCfg, ImplicitActuatorCfg
...
class GapONetBenchmark:
    ...
    def set_motion(self, motion_file, motion_name):
        ...
        # Extract joint names from full paths
        # (e.g., "/h1_2/torso_link/left_shoulder_pitch_joint" -> "left_shoulder_pitch_joint")
    ...
    def _init_logger(self):
        ...
        output_folder = os.path.join(self.output_folder, self.robot_name, self.motion_name)
    ...
    def run_benchmark(self):
        ...
        if abs(actual_pos_after_buffer).max() > 10.0:
            log_message("ERROR: Joint positions exploded during buffer phase!")
            log_message(f"Max position: {abs(actual_pos_after_buffer).max():.2f} rad")
            log_message("This indicates actuator instability. Aborting...")
            return
    ...
    def log_joint_properties(self):
        ...
        log_message("  Control mode: Position control (set_joint_position_target)")
        log_message(f"  PD gains: kp={self.kp}, kd={self.kd}")
        if self.use_gaponet:
            log_message("  Actuator: GapONetActuatorCfg")
            log_message(f"    - Neural network model: {self.gaponet_model}")
            log_message("    - Network computes torques from position targets")
        else:
            log_message("  Actuator: ImplicitActuatorCfg")
            log_message("    - PhysX native PD controller (C++, highly stable)")
