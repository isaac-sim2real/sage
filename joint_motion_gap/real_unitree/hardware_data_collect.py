...
from robot_config import get_robot_config, print_robot_info, list_available_robots

# ROS2 imports
import rclpy
from rclpy.node import Node
from unitree_hg.msg import LowState, LowCmd, MotorCmd
...
class MotionManager:
    ...
    def _load_current_motion(self) -> None:
        ...
        print(f"Loaded motion: {motion_name} ({self.interpolated_data.shape[0]} frames, {self.interpolated_data.shape[0]/self.control_frequency:.1f}s)")
    ...
    def _interpolate_motion(self, dof_data: np.ndarray, fps: int, target_fps: int) -> np.ndarray:
        ...
        for dof_idx in range(dof_data.shape[1]):
            interpolator = interp1d(original_time, dof_data[:, dof_idx], 
    ...
    def _handle_motion_completion(self) -> None:
        ...
        if self.robot_config:
            self.current_motion_info.last_frame_position = self.interpolated_data[self.current_motion_info.frame_counter - 1][self.robot_config.upper_body_joints]
...
class MultiRobotMotionControlNode(Node):
    ...
    def __init__(self, robot_config, output_dir):
        ...
        print("Controls: L1=Emergency stop, B=Start/Stop motion, A=Decrease speed, Y=Increase speed, X=Reset speed, R1=Exit")
        ...
        if len(time1_valid) <= len(time2_valid):
            ...
            for key in ['angle_rad', 'velocity_rads', 'current']:
                ...
                if data2_array.ndim == 2:  # (n_samples, n_joints)
                    ...
                    for joint_idx in range(n_joints):
                        ...
                        interp_func = interp1d(time2_valid, data2_array[:, joint_idx], 
                                             kind='linear', bounds_error=False, fill_value='extrapolate')
                        interpolated_data[:, joint_idx] = interp_func(target_time)
                    
                    sync_data2[key] = interpolated_data
                else:  # 1D array
                    interp_func = interp1d(time2_valid, data2_array, 
                                         kind='linear', bounds_error=False, fill_value='extrapolate')
            ...
            for key in ['angle_rad', 'velocity_rads', 'current']:
                ...
                if data1_array.ndim == 2:  # (n_samples, n_joints)
                    ...
                    for joint_idx in range(n_joints):
                        # Use linear interpolation
                        interp_func = interp1d(time1_valid, data1_array[:, joint_idx], 
                                             kind='linear', bounds_error=False, fill_value='extrapolate')
                        interpolated_data[:, joint_idx] = interp_func(target_time)
                    
                    sync_data1[key] = interpolated_data
                else:  # 1D array
                    interp_func = interp1d(time1_valid, data1_array, 
                                         kind='linear', bounds_error=False, fill_value='extrapolate')
    ...
    def save_motion_csv_files(self, output_dir, motion_name, command_time_list, command_val_list, 
                             robot_joint_names, data1, data2):
        """
    ...
    def stop_recording(self):
        ...
        # Get motion name
        motion_name = self.motion_manager.current_motion_info.name if self.motion_manager.current_motion_info else "unknown_motion"
