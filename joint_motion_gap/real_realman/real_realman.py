...
def save_motion_csv_files(output_dir, motion_name, command_time_list, command_val_list, 
...
def collect_motion_data_from_txt(collector: HumanoidDualArmCollector, txt_file_path, save_device_dir_path=None, slowdown_factor=1, payload=0):
    """
    ...
    # Interpolate command
        ...
        for j in range(steps_per_motion):
            ...
            if (i % 50 == 0) and (j == 0):
                print(f"Motion Step {i}/{n_steps} | Control {j+1}/{steps_per_motion} | Loop cost: {elapsed:.4f}s | sleep: {sleeptime:.4f}s")
...
def real_realman_main(txt_file_path, save_data_path):
    ...
    collector = HumanoidDualArmCollector(
        arm1_ip, arm2_ip, arm1_port, arm2_port, recv_port1, recv_port2
    )
